...
import { BillingInfo, NewOrder, PaymentInfo, ShippingInfo } from "@/interfaces";
...
const ReviewForm: React.FC<ReviewFormProps> = ({ onNext, onBack }) => {
  const { data: session } = useSession();
  const stripe = useStripe();
  const router = useRouter();
  // Set default values for billingInfo and shippingInfo
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      const newOrder: NewOrder = {
        ...
        deliveryType: shippingInfo?.deliveryMethod || "FREE_STANDARD",
        shippingInfo: shippingInfo,
        billingInfo: billingInfo,
